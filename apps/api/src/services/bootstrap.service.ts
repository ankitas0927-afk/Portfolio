...
export async function ensurePortfolioSeed(adminId: string) {
  ...
  // After the initial seed or explicit legacy/placeholder repair, runtime should not
  // repopulate database records during ordinary requests. Admin CRUD stays the source of truth.
  return { seeded: false, repairedPlaceholders: false, ensuredMissingContent: false };
}
